...
async def get_all_registrations_global(
    ...
    event_id: UUID = Query(default=None),
    status: str = Query(default=None),
    session: AsyncSession = Depends(get_db),
    admin: User = Depends(get_admin),
):
    registrations = await registration_service.list_all_registrations(
        page, limit, search, event_id, status, session
    )
...
@admin_router.patch('/{event_id}/upload-video-url')
...
@admin_router.patch('/{event_id}/publish')
...
@admin_router.patch('/{event_id}/complete')
...
@admin_router.get('/{event_id}/registrations', response_model=RegistrationList)
...
async def event_analytics(event_id: UUID, session: AsyncSession = Depends(get_db)):
